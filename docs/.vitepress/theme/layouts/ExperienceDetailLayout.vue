...
<template>
  <div class="atlas-site">
    ...
    <main class="container page-container">
      <a class="back-link" :href="url('/experience/')">← 返回实习与实践经历</a>
      <header class="experience-detail-header"><div><span class="experience-detail-icon"><ItemIcon :name="frontmatter.icon" :size="28" /></span><span class="section-index">INTERNSHIP / PRACTICE DETAIL</span><h1>{{ frontmatter.title }}</h1><p>{{ frontmatter.summary }}</p></div><div class="experience-detail-meta"><span>{{ frontmatter.organization }}</span><span>{{ frontmatter.period }}</span><span>{{ frontmatter.location }}</span></div></header>
