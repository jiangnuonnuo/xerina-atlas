...
<template>
  <div class="atlas-site">
    ...
    <main class="container doc-layout">
      <aside class="doc-sidebar"><div class="doc-sidebar-head"><span>DOCUMENT TREE</span><span>{{ projectChapters.length + 1 }} FILES</span></div><nav class="doc-tree"><a class="doc-tree-link" :class="{ 'is-active': active(project?.url || '') }" :href="withBase(project?.url || '/')"><span>00</span><strong>项目总览</strong></a><a v-for="(chapter, index) in projectChapters" :key="chapter.slug" class="doc-tree-link" :class="{ 'is-active': active(chapter.url) }" :href="withBase(chapter.url)"><span>{{ String(index + 1).padStart(2, '0') }}</span><strong>{{ chapter.frontmatter.title }}</strong></a></nav><div class="doc-sidebar-note"><span>READING PATH</span><p>先了解问题和目标，再进入系统设计、核心实现与验证结果。</p></div></aside>
