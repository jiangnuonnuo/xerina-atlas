...
function normalizePath(url: string) {
  const path = decodeURI(url).replace(/index\.html$/, '').replace(/\.html$/, '').replace(/\/$/, '')
  return path || '/'
}

function active(url: string) { return normalizePath(currentPath.value) === normalizePath(url) }
</script>
...
<template>
  <div class="atlas-site">
    ...
    <div v-if="project" class="doc-project-bar">
      <div class="container doc-project-bar-inner"><a class="doc-project-identity" :href="url(project.url)"><span class="doc-project-icon"><ItemIcon :name="project.frontmatter.icon" :size="20" /></span><span>PROJECT DOCUMENT</span><strong>{{ project.frontmatter.title }}</strong></a><nav class="doc-top-nav" aria-label="项目文档章节"><a :class="{ 'is-active': active(project.url) }" :href="url(project.url)">总览</a><a v-for="chapter in projectChapters" :key="chapter.slug" :class="{ 'is-active': active(chapter.url) }" :href="url(chapter.url)">{{ chapter.frontmatter.title }}</a></nav></div>
    </div>
    <main class="container doc-layout">
      <aside class="doc-sidebar" :class="{ 'is-open': sidebarOpen }"><div class="doc-sidebar-head"><span>DOCUMENT TREE</span><button class="doc-sidebar-toggle" type="button" @click="sidebarOpen = !sidebarOpen">{{ sidebarOpen ? '收起目录' : '展开目录' }}<span aria-hidden="true">{{ sidebarOpen ? '−' : '+' }}</span></button><span class="doc-sidebar-count">{{ projectChapters.length + 1 }} FILES</span></div><nav class="doc-tree"><a class="doc-tree-link" :class="{ 'is-active': active(project?.url || '') }" :href="url(project?.url || '/')" @click="sidebarOpen = false"><span>00</span><strong>项目总览</strong></a><a v-for="(chapter, index) in projectChapters" :key="chapter.slug" class="doc-tree-link" :class="{ 'is-active': active(chapter.url) }" :href="url(chapter.url)" @click="sidebarOpen = false"><span>{{ String(index + 1).padStart(2, '0') }}</span><strong>{{ chapter.frontmatter.title }}</strong></a></nav><div class="doc-sidebar-note"><span>READING PATH</span><p>先了解问题和目标，再进入系统设计、核心实现与验证结果。</p></div></aside>
