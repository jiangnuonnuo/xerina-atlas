...
如果只关心一次真实运维调用，推荐阅读顺序是 [SSH 连接认证与主机信任](./05-SSH连接认证与主机信任.md) → [SSH 命令执行主链路](./06-SSH命令执行主链路.md) → [长执行输出游标与故障恢复](./07-长执行输出游标与故障恢复.md)。如果关心 Agent 如何驱动这条链路，再接着读 [ReAct 对话与 Agent 调用链](./10-ReAct对话与Agent调用链.md)。
