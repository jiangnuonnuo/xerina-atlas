...
group: 安全与隔离
order: 11
description: 区分普通 HTTP、MCP Bearer、userId、Agent 会话、执行上下文、连接凭据和远端账户各自的隔离责任。
sidebar: true
layout: project-doc
---
...
连接配置和聊天事实存放在 MySQL，执行元数据与输出放在 Redis，JSch、终端、绑定和上下文状态留在应用 JVM。Docker Compose 只把应用、MySQL 和 Redis 放在后端 bridge 网络，远端 SSH 主机在 Compose 网络之外；应用通过 SSH 访问目标主机，再在目标主机上调用 Docker。这个拓扑把“应用依赖”与“运维目标”分开，也意味着目标主机的网络能力、Docker daemon 和 SSH 权限不能由 Compose healthcheck 代替。

普通 HTTP、MCP 和内置 Agent 共享 Domain/Case 规则，但不是共享同一个认证入口：普通 HTTP 没有发现统一登录过滤器，MCP 只由 `/mcp` 路径上的静态 Bearer 过滤器保护，内置 Agent 是进程内调用，不经过 HTTP 认证。生产网络应禁止客户端绕过网关直达普通 API，并在资源用例层补充可信主体到 connectionId、sessionId 和 executionId 的归属校验。
