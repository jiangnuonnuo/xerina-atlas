...
而公司的报表业务字段是和平台的页面字段进行映射的，也就是根据平台字段的指标去写sql 语句找出数据量，生成报表写作，但是平台页面是会变化的，数仓的字段维护无法实施同步更新，也不可擅自改动，于是需要维护字段映射的平台和报表采集业务，
后续平台页面的变成通过树级结构，很好的同步维护更新
...
- **采集困难**：如果纯人工去页面核对采集，公司所需要的平台超 15 + ，总共字段维护超100w + 数据量，纯人工难度大，不好定位。
...
字段目录本质是树。用关系表存父子，写入简单但祖先链查询贵；用文档树存整棵，查询快但并发写入难。但是无论是查询还是修改，对于日常使用都是常用的接口信息，所以必须折中
方案取中间路径：**直接父级关系 + 祖先链按需展开**，并维护结构不变量。
### 4. 交付：同步返回 vs 异步导出

字段字典可能很大、消费方很多。同步返回会拖垮写入链路，所以用**独立导出任务**：从一致性快照流式写入，状态可查、失败可重试、交付原子，将导出任务与平常的日常操作进行分离，用线程池去分配线程，避免大量的线程并发，产生 mysql 的备份过多
从而导致 mysql 的连接池打满
