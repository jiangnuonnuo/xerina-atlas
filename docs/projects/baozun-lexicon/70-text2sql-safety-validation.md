...
当前 Cognida 已具备对话式 Data Agent、语义查询、只读 SQL、Result Store、分析和 Text2SQL 评测能力；企业的定时调度、消息推送、审批和正式报表发布仍需由现有调度平台或新增报表任务模块承接。

## 10. 最终边界

这项能力可以安全地用于：

- 让业务人员用自然语言提出日报周报查询需求；
- 让 Data Agent 先查 Schema，再生成 SQL；
- 让高频指标优先走统一语义口径；
- 让未覆盖问题在只读、限量、超时边界内回退查询；
- 让结果通过 `result_id` 进入分析、图表和报告；
- 让 Golden Query、结果校验和人工审核保障正式报表质量。

它不能被描述成：

- 模型可以直接修改、删除或写入公司的生产数仓；
- SQL 能执行就代表业务结果一定正确；
- 自然语言已经替代了指标治理、数据开发和业务审核；
- 当前对话式能力已经自动完成每天的正式日报推送。

我认为最关键的工程原则是：**AI 负责理解问题和提出查询，语义层负责统一口径，后端负责安全执行，代码负责计算和校验，人工负责正式发布。**
