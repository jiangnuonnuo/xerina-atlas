...
前几篇把 Lexicon 从页面采集讲到字段目录和 XLSX 交付。字典交出去之后，业务词仍然只是静态映射。50 / 51 把字典升级成可执行语义；55 / 60 / 70 再接到 Cognida 的日报周报问数：不是再包一层会写 SQL 的 ChatBI，而是让已治理口径走确定性引擎，未覆盖才诚实回退 Text2SQL。当前是对话按需出数，定时推送日报仍需企业调度承接。

### 50 · 从字段字典到可执行语义

说明页面字段字典为什么还不够：它能回答业务词对应哪一列，但不能回答怎么聚合、怎么关联、怎么翻译枚举。文章把字典行映射到逻辑表、度量、指标、维度、值映射和关系，并划清语义模型不生成 SQL、不替代数仓、不让对话现场发明口径。

### 51 · 语义对象、流转与目录卡

说明六个对象如何围绕逻辑表展开，指标和度量为何分开，建模写入与查询读取如何共用版本锚点，以及为什么问数入口只能看到业务目录卡，物理表、表达式和 JOIN 只给编译侧。

### 55 · 治理型语义层与日报周报查询架构

从实习生面试口播切入：分析师每周手写周报 SQL，真正难的是口径一致。文章拆开 LLM、语义引擎、只读执行和分析代码的职责，给出治理主路与词法回退、六点契约、Lexicon 到物理字段的绑定，以及「不能说成已上线」的边界。

### 60 · 从一句周报到受治理 SQL

按「从 0 讲到 1」走通：意图路由到 report → 拆成总览/趋势/分群 → 模型只填指标和维度 → `metricsql` 确定性写 SQL → `sql_execute` 得到 `result_id` → 代码算对比和趋势。未覆盖才回退 `get_schema`。附 STAR 口播。

### 70 · SQL 安全边界、正确性验证与面试追问

说明只读闸门、数据源路由、结果信封、Golden Query、有限自修复，以及正式报表的生成/运行两段生命周期。收束高频追问：为什么不让模型直接写周报 SQL、和 ChatBI 的差别、如何证明生成正确。

这几篇的核心链路是：

> 页面字段字典 → 可执行语义模型 → 目录卡给问数入口 → 引擎生成 SQL（未覆盖再 Text2SQL）→ 只读执行 → 代码算数 → 审核后的日报 / 周报。
