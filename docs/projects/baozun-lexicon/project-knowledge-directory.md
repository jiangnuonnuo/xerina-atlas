...
前几篇把 Lexicon 从页面采集讲到字段目录和 XLSX 交付。字典交出去之后，业务词仍然只是静态映射。后面按三层接 Cognida 问数，不要把 55 的面试总览算成第四层。

### 语义模型 · 50 / 51

**50 · 从字段字典到可执行语义**：页面字段字典为什么还不够；一行字典如何变成逻辑表、度量、指标、维度、值映射和关系。语义模型不生成 SQL、不替代数仓、不让对话现场发明口径。

**51 · 语义对象、流转与目录卡**：六个对象如何围绕逻辑表展开，整份写入如何生效，查询如何加载某一个模型包，以及为什么对话只能看到目录卡。本层停在「完整模型包交给编译」。SQL 复用和五步编译已移出。

### 执行引擎 · 60 / 61 / 62

**60 · 编译步骤如何使用语义模型**：接上 51 的模型包，按索引、维度、指标、时间、过滤说明每一步读哪些对象、产出什么、怎样记未覆盖。完整 SQL 在 61。

**61 · 从 Agent Query 到最终 SQL**：钉死 Agent 只能返回名称和过滤值；用「上周华东区 GMV，按天」从 Query 逐步装配出完整 SQL。

**62 · 覆盖、JOIN 与 SQL 装配**：未覆盖必须空 SQL；跨表只走已登记的 to-one 边；零件如何落进各子句；按模型版本复用。

### Agent 循环 · 71 / 72

**71 · 从一句人话到只读结果**：接上 62 的两种出口，用同一句人话走完意图分类、目录卡、`semantic_query`、只读执行和结果信封。

**72 · 未覆盖回退、只读闸门与周报拆章**：实时库存必须换 `get_schema`；词法 SQL 仍走只读闸门；受信缓存由工具层取放；周报先拆章再各自取数。

### 不占三层

**55 · 治理型语义层与日报周报查询架构**：面试 90 秒口播，讲职责拆分。正文按 50–51、60–62、71–72 读。

核心链路：

> 页面字段字典 → 语义模型（目录卡给问数入口）→ 执行引擎生成 SQL（未覆盖则空）→ Agent 只读执行或回退 → 代码算数 → 审核后的日报 / 周报。
