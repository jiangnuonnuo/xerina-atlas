...
在平台出现之前，字段字典靠业务人员逐字段录入 Excel 表格文件维护，有两个固有问题：字段在表格里没有页面出处，不好定位与核对；层级关系无法直观表达，页面一改版整份表格就过期。平台把这件事拆成人机协作的闭环——**业务人员负责确定页面状态与业务范围，用插件把当前选区清洗成结构化证据，上传到平台的输入接口；内部 Agent 只负责判断业务含义和真正的字段层级。**
...
DOM-SCOUT 把最需要业务判断的动作交给业务人员：业务人员知道当前要采集的是“退款信息”还是“物流信息”，因此只选择相关区域，从而把“采什么、采到哪算完整”的责任收回到人身上；区域内的层级结构与带业务名称的标签信息，才会进入内部 Agent 做语义判断。
...
DOM-SCOUT 更贴合这条操作路径。上游项目提供页面内高亮、多选、Parent / Child 导航、Clean HTML、Structure、Accessibility、Selectors 和 Token 预估等通用能力。评估时固定上游版本和提交，内部只围绕字段证据格式、安全清洗和 Agent 输入协议做定制。
...
还有一个结构性优势容易被低估：采集发生在业务人员**自己已登录、已打开的浏览器会话**里——登录动作由人完成，插件不读取 Cookie、请求头或网络流量（见第 10 节），也不需要像 Agent + Playwright 自动化那样自行维护登录态；页面由真实用户会话打开，不存在“自动化驱动被风控识别”的对抗问题，对防护较强的页面也能稳定读取用户确认选区内的已渲染 DOM。
...
| 风控 / 防护较强的页面 | 自动化驱动（Playwright）易被风控识别，或拿不到完整 DOM | 由业务人员已登录的真实会话打开页面，插件只读取用户确认选区内的已渲染 DOM，不做自动化遍历 |
