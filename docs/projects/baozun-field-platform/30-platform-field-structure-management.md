...
当前 schema 使用 MySQL 8.0+、InnoDB、`utf8mb4`，不创建物理外键。 `platform_id、parent_id、ancestor_id、descendant_id` 是逻辑外键，由服务事务和定期巡检 SQL 保证跨表一致性。这样保留批量导入、闭包重建和历史数据修复的操作空间，但所有写入口必须经过领域服务，不能让任意 SQL 绕过规则。
...
- 当前 V2 对外节点修改接口主要修改名称和类型，MyBatis 已提供详情 `upsert` 能力；若开放详情命令，必须接入本节的节点锁、CAS 和版本发布协议，不能让控制器直接调用 mapper。

### 3.3 移动子树

移动是最容易产生环、脏闭包和交叉锁等待的操作。当前执行顺序固定：

1. 使用非锁定闭包查询读取移动节点子树 ID；读取新父节点快照（如果有），这些数据只用于形成候选集合。
