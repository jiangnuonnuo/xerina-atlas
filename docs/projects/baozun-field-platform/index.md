...
featured: true
status: active
summary: 把业务页面上的菜单、页签、分组、字段与指标沉淀为可维护的层级目录，通过 DOM-first 字段采集、受限 Agent 层级解析、一致性快照与异步导出形成完整字段治理闭环。
role: 字段目录平台｜全栈研发
stack:
  - Java
  - Spring Boot
...
> 这是一份字段目录平台的全栈技术专栏。它系统拆解字段证据采集、受限 Agent 层级解析、交互工作台、目录树事务并发、异步字典交付和页面变化治理。
