...
整套专栏只保留六篇实现文章：`01、10、20、25、30、40`。术语、API、错误、测试、安全、性能和故障处理都放入对应功能章节，不再拆成独立的空泛章节。
...
本章的硬约束是：草稿渲染可以接近当前树形结构，但草稿在业务人员确认前不得进入数据库正式目录。
...
六篇文章按以下顺序形成一条可实现、可审计的技术叙事：

![技术文章阅读顺序](./assets/reading-sequence.svg)

每个环节都要回答四个问题：数据当前属于哪个状态、谁可以修改、基于哪个版本、失败后从哪里恢复。满足这四点，页面采集、Agent 解析、树形草稿、正式目录和字段字典才构成同一条可信工程链路。
