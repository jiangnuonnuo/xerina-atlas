...
根工程是 Maven 多模块项目，Java 编译目标是 17，Spring Boot 应用模块是 `ai-mcp-gateway-app`。模块依赖关系不是多个独立服务，而是一份最终由 App 启动的分层单体：`api` 提供契约，`trigger` 提供入口，`case` 组织场景，`domain` 保留业务边界，`infrastructure` 连接 MySQL、Redis、HTTP、MCP 和 LLM。
