...
因此，AI MCP Gateway 更准确的定位是：**面向企业内部能力的 AI 工具接入与发布平台**。它兼有协议适配器、能力工作台、运行时网关和 AI 联调入口，但当前实现仍然是一个按领域分层的单体应用，不应包装成已经拆分完成的微服务平台。

![AI MCP Gateway 运行时边界](./assets/architecture.gif)

图要回答的问题：管理面、MCP 运行面、领域规则、持久化状态和外部能力各自在哪里。图中的不同连线代表配置、请求、运行时读取和外部调用，不是把整个系统简化成一条 Controller 调用链。
...
| 认证与隔离 | 包级 Bearer Token、旧 API Key 兼容、限流、网关和会话校验 | 新认证会话必须绑定能力包和 token 身份 |
| AI 联调 | Spring AI 发现 MCP 工具、自动工具调用、调用轨迹和结果校验 | LLM 负责选择工具，网关负责批准执行 |
| 运行时一致性 | MySQL 事实、Redis 投影、脏标记、版本键、回源锁 | Redis 故障不能被解释成能力为空 |
| 从 0 到 1 落地 | 按真实样例完成环境、接入、发布、MCP、LLM 和验收 | 每一步都保存服务端事实，不能只看页面状态 |
...
这里最容易被忽略的是“测试通过”和“已发布”不是同一个事实。测试保证一份配置在某个时刻能够执行；发布把这份配置冻结为客户端可见的版本。配置变化后，旧测试指纹失效，客户端继续看到旧发布内容，直到新的配置重新测试并发布。
...
图要回答的问题：依赖为什么朝向 Domain，外部资源为什么只能通过端口进入。管理端和 MCP 端虽是两组入口，但都复用 Domain 的能力包、工具调用和数据源安全规则。
...
| 测试工具链 | 核心真实链路已验收，Java 25 下部分 Mockito/Byte Buddy 测试受版本限制 | 固定受支持 JDK 或升级测试依赖后跑全量回归 |

这套边界不是文章的免责声明，而是部署、验收和企业审批时必须成为正式设计输入的事实。
