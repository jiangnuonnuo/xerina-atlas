...
const records = filesUnder(docsRoot).map((file) => {
  const parsed = matter(fs.readFileSync(file, 'utf8'))
  return { file, content: parsed.content, data: parsed.data }
})
const published = records.filter(({ data }) => data.draft !== true)
const urls = new Map<string, string>()

for (const { file, content, data } of published) {
  ...
  if (relative.startsWith('projects/') && relative !== 'projects/index.md' && data.layout !== 'project-doc') {
    errors.push(`${file}: 项目总览和章节必须使用 layout: project-doc`)
  }
  if (data.layout === 'project-doc' && /^#\s+/m.test(content)) {
    errors.push(`${file}: project-doc 正文不要重复使用一级标题，页面标题由 frontmatter.title 统一渲染`)
  }
